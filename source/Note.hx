...
class Note extends FlxSprite
{
	...
	private function set_noteType(value:String):String {
		...
		if (noteData > -1 && noteData < PlankPrefs.data.arrowHSV.length)
		{
			colorSwap.hue = PlankPrefs.data.arrowHSV[noteData][0] / 360;
			colorSwap.saturation = PlankPrefs.data.arrowHSV[noteData][1] / 100;
			colorSwap.brightness = PlankPrefs.data.arrowHSV[noteData][2] / 100;
		}
	...
	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?inEditor:Bool = false)
	{
		...
		x += (PlankPrefs.data.middleScroll ? PlayState.STRUM_X_MIDDLESCROLL : PlayState.STRUM_X) + 50;
		// MAKE SURE ITS DEFINITELY OFF SCREEN?
		y -= 2000;
		this.strumTime = strumTime;
		if(!inEditor) this.strumTime += PlankPrefs.data.noteOffset;
		...
		if (isSustainNote && prevNote != null) {
			...
			if(PlankPrefs.data.downScroll) flipY = true;
