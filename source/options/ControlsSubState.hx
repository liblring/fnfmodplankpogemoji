...
class ControlsSubState extends MusicBeatSubstate {
	...
	override function update(elapsed:Float) {
		if(!rebindingKey) {
			...
			if (keyPressed > -1) {
				var keysArray:Array<FlxKey> = PlankPrefs.data.keyBinds.get(optionShit[curSelected][1]);
	...
	private function addBindTexts(optionText:Alphabet, num:Int) {
		var keys:Array<Dynamic> = PlankPrefs.data.keyBinds.get(optionShit[num][1]);
	...
	function reloadKeys() {
		...
		trace('Reloaded keys: ' + PlankPrefs.data.keyBinds);
