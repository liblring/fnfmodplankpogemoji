...
class VisualsUISubState extends BaseOptionsMenu
{
	public function new()
	{
		...
		var option:Option = new Option('Pause Screen Song:',
			...
			'string',
			'Tea Time',
			['None', 'Breakfast', 'Tea Time', 'mnalking rn']);
		addOption(option);
	...
	{
		if(PlankPrefs.data.pauseMusic == 'None')
			FlxG.sound.music.volume = 0;
		else
			FlxG.sound.playMusic(Paths.music(Paths.formatToSongPath(PlankPrefs.data.pauseMusic)));
	...
	{
		if(Main.fpsVar != null)
			Main.fpsVar.visible = PlankPrefs.data.showFPS;
	}
	#end
}
